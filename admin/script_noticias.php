...
$resizeObj = new resize($uploaddir.$foto);
 
// *** 2) Resize image (options: exact, portrait, landscape, auto, crop)
$resizeObj -> resizeImage(600, 500, 'crop');
 
// *** 3) Save image
$resizeObj -> saveImage($uploaddir.'resized-'.$foto, 100);
...
$query = "insert into tbl_galeria_pet(titulo, legenda, foto) values ('$titulo', '$legenda', 'resized-$foto')";
$rs= mysql_query($query);
...
$query = "insert into tbl_galeria(titulo, legenda, foto) values ('$titulo', '$legenda', 'resized-$foto')";
$rs= mysql_query($query);
...
// *** 1) Initialize / load image
$resizeObj = new resize($uploaddir.$nome_arquivo);
 
// *** 2) Resize image (options: exact, portrait, landscape, auto, crop)
$resizeObj -> resizeImage(575, 260, 'crop');
 
// *** 3) Save image
$resizeObj -> saveImage($uploaddir.'resized-'.$nome_arquivo, 100);		   
...
$query = "insert into tbl_noticias(id_categoria_noticia, titulo, descricao, destaque, fotografo, legenda, msg, nome_arquivo, data, status) values ('4', '$titulo', '$descricao', '$destaque', '$fotografo', '$legenda', '$msg', 'resized-$nome_arquivo', '$data_publicacao', '$status')";
$rs= mysql_query($query);
...
$nome_arquivo = $nome_arquivo_file['name'];
if(isset($tmp))
$tmp = $nome_arquivo_file['tmp_name'];
if(!empty($nome_arquivo)){
        ...
        unlink ($uploaddir_antigo.$nome_arquivo_antigo);
         ...
         move_uploaded_file($_FILES['nome_arquivo']['tmp_name'], $uploaddir_antigo . $nome_arquivo);
		 
		 // *** 1) Initialize / load image
		$resizeObj = new resize($uploaddir_antigo.$nome_arquivo);
 
		// *** 2) Resize image (options: exact, portrait, landscape, auto, crop)
		$resizeObj -> resizeImage(575, 260, 'crop');
 
		// *** 3) Save image
		$resizeObj -> saveImage($uploaddir_antigo.'resized-'.$nome_arquivo, 100);
         $query = "update tbl_noticias SET nome_arquivo = 'resized-$nome_arquivo' where id_noticia='$id_noticia'";
         $rs= mysql_query ($query);
        }
